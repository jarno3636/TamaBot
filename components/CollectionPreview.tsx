...
export default function CollectionPreview() {
  ...
  async function refresh() {
    ...
    // client-side timeout (prevents endless spinner)
    ...
    try {
      ...
      if (next.length === 0) {
        setError("No mints found yet.");
      }
    } catch (e: any) {
      if (e?.name === "AbortError") {
        setError("Request timed out. Tap Refresh again.");
      } else {
        const msg = (e?.message || "Base RPC temporarily unavailable").trim();
        setError(msg);
      }
    } finally {
    ...
    <section className="w-full flex justify-center">
      <div className="glass glass-pad w-full max-w-md sm:max-w-lg md:max-w-2xl">
        ...
        {error && (
          <div className="mt-3 rounded-xl border border-white/10 bg-black/30 p-3">
            <div className="max-h-28 overflow-auto">
              <p className="text-[11px] text-rose-300 break-words whitespace-pre-wrap">
                {error}
              </p>
            </div>
          </div>
        ...
        <p className="mt-1 text-center text-[11px] text-white/45">
          Rendered from on-chain tokenURI SVG (no log scanning).
        </p>
