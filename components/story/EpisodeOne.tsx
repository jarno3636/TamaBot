...
type SaveShape = {
  ...
  // flags are what future episodes can branch on
  flags: {
    complied: boolean;
    cautious: boolean;
    adversarial: boolean;
  ...
  // this is “what the game thinks you are”
  profile: {
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div
              className="mt-2 text-[15px] md:text-[16px] font-extrabold"
        ...
        <div className="mt-3 text-[11px]" style={{ color: "rgba(255,255,255,0.46)" }}>
          Tip: your choice becomes a permanent flag once we turn on-chain saves on.
        </div>
      </div>
    </button>
  );
...
export default function EpisodeOne({
  ...
  const [phase, setPhase] = useState<"intro" | "signal" | "choice" | "result">(
  ...
  useEffect(() => {
    ...
    // auto resolve to default if time expires
    resolveChoice(defaultChoice, { expired: true });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [secondsLeft, phase]);

  // hotkeys during choice
  useEffect(() => {
    if (phase !== "choice") return;
  ...
  function buildSave(choiceId: EpisodeOneChoiceId): SaveShape {
    // flags + profile: make these meaningful and branchable
    const baseFlags = {
      complied: false,
      cautious: false,
      adversarial: false,
  ...
  function resolveChoice(choiceId: EpisodeOneChoiceId, opts?: { expired?: boolean }) {
    setPicked(choiceId);
  ...
  const scene = useMemo(() => {
    // Dark/edgy narrative copy: meaningful, not gimmicky.
    // 4th-wall hint: “it’s reading your hesitation”.
    return {
      intro: {
      ...
      choice: {
        ...
        warning:
          "If the timer hits zero, the system will interpret hesitation as intent and choose for you.",
      },
      result: {
        heading: "STATE COMMITTED",
      },
    };
  }, []);
  ...
  return (
    <section
      ...
      style={{
        borderColor: "rgba(255,255,255,0.10)",
        background:
          "linear-gradient(180deg, rgba(2,6,23,0.92), rgba(2,6,23,0.68))",
          ...
          <div className="mt-6">
            <h2 className="text-[18px] md:text-[22px] font-extrabold tracking-tight" style={{ color: "rgba(255,255,255,0.94)" }}>
              {phase === "intro"
                ? scene.intro.heading
                : phase === "signal"
                ? scene.signal.heading
                : scene.result.heading}
            </h2>

            <div className="mt-3 grid gap-2">
              {(phase === "intro" ? scene.intro.body : phase === "signal" ? scene.signal.body : []).map((line, i) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      borderColor: "rgba(255,255,255,0.10)",
                      background:
                        "radial-gradient(760px 260px at 20% 0%, rgba(168,85,247,0.14), transparent 60%), radial-gradient(720px 260px at 90% 20%, rgba(56,189,248,0.10), transparent 62%), rgba(255,255,255,0.04)",
                    }}
                  >
                    <div className="text-[14px] font-extrabold" style={{ color: "rgba(255,255,255,0.92)" }}>
                      {save.artifact.name}
                    </div>
                    <div className="mt-2 text-[12px] leading-relaxed" style={{ color: "rgba(255,255,255,0.70)" }}>
                      {save.artifact.desc}
                    </div>

                    <div className="mt-4 text-[11px]" style={{ color: "rgba(255,255,255,0.52)" }}>
                      This artifact is a continuity hook. Later episodes can require it, corrupt it, or trade it.
                    </div>
                  </div>
                  ...
                  >
                    Next: Episode 2 will read your flags and react.
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mt-6 flex flex-wrap items-center gap-2">
              {phase === "intro" && (
                <button
                  ...
                  style={{
                    border: "1px solid rgba(255,255,255,0.12)",
                    background:
                      "linear-gradient(90deg, rgba(56,189,248,0.92), rgba(168,85,247,0.70))",
              ...
              {phase === "signal" && (
                <button
                  ...
                  style={{
                    border: "1px solid rgba(255,255,255,0.12)",
                    background:
                      "linear-gradient(90deg, rgba(251,113,133,0.92), rgba(168,85,247,0.72))",
                    color: "rgba(2,6,23,0.98)",
              ...
              {phase === "result" && (
                <>
                  <button
                    type="button"
                    onClick={() => {
                      // allow replay/reset episode 1 for testing (admin vibe)
                      try {
                  ...
                  <button
                    ...
                    style={{
                      border: "1px solid rgba(255,255,255,0.12)",
                      background:
                        "linear-gradient(90deg, rgba(52,211,153,0.92), rgba(56,189,248,0.82))",
                      color: "rgba(2,6,23,0.98)",
              ...
              <div className="text-[11px] ml-1" style={{ color: "rgba(255,255,255,0.46)" }}>
                ...
                {phase === "signal"
                  ...
                  : phase === "intro"
                  ? "The cartridge feels warm before you touch it."
                  : phase === "result"
                  ? "A choice saved is a future you can’t pretend you didn’t choose."
                  : ""}
              </div>
            </div>
          </div>
          ...
          <div className="mt-6">
              ...
              >
                ...
                <div className="mt-3">
                  ...
                  <div className="mt-3 text-[11px]" style={{ color: "rgba(255,255,255,0.50)" }}>
                    Hotkeys: 1 Accept • 2 Stall • 3 Spoof • 4 Pull Plug
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <ChoiceCard
                ...
                reward="You gain access pathways: faster rewards, easier permissions, fewer locks."
              ...
              <ChoiceCard
                ...
                reward="You keep yourself clean—at least for now. You deny the system a narrative."
                onPick={() => resolveChoice("PULL_PLUG")}
              />
            </div>

            <div className="mt-6 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.46)" }}>
              “This isn’t urgency. It’s a test for the kind of person you become when the clock is real.”
            </div>
          </div>
        )}
      </div>
