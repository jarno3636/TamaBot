...
export default function TamaBotClient({ id }: { id: number }) {
  ...
  const { data: sRaw, isLoading: loadingState, refetch } = useReadContract({
    ...
    query: { enabled: validId, refetchOnWindowFocus: false } as any,
  });
    ...
    <div className="w-full max-w-3xl mx-auto px-5 pt-6 pb-16">
      {!validId ? (
        ...
        <section className="stack">
          ...
          {/* Live stats + interactive care */}
          <Card className="glass glass-pad">
            <div className="mb-3 pill-row">
              <Pill>Live stats</Pill>
              <Pill>Interact to change</Pill>
            </div>

            {loadingState && <div className="text-sm text-white/80">Loading on-chain state…</div>}
