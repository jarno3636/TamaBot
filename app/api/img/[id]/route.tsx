...
export async function GET(_req: Request, ctx: any): Promise<Response> {
  ...
  const eyesIdx  = rint(seed, 1, 5);
  const mouthIdx = rint(seed, 2, 5);
  const faceIdx  = rint(seed, 3, 4);
  const armIdx   = rint(seed, 4, 3);
  const legIdx   = rint(seed, 5, 3);
  const bgIdx    = rint(seed, 6, 6);
  const antenna  = rbool(seed, 7);

  const [rx] = radii(faceIdx);
  ...
  const BgLayer = () => {
    if (bgIdx === 1) {
      ...
      return (<><Rect x={0} y={0} w={SIZE} h={SIZE} fill={bg} />{lines}</>);
    }
    if (bgIdx === 2) {
      return (
        <>
    ...
    if (bgIdx === 3) {
      return (
        <>
          <Rect x={0} y={0} w={SIZE} h={SIZE} fill={bg} />
          <Rect x={-200} y={SIZE - 280} w={SIZE + 400} h={180} fill={stroke} opacity={0.08} r={12} shadow={`0 0 50px ${stroke}20`} />
        </>
      );
    }
    if (bgIdx === 4) {
      ...
      return (<><Rect x={0} y={0} w={SIZE} h={SIZE} fill={bg} />{dots}</>);
    }
    if (bgIdx === 5) {
      return (
        <>
          <Rect x={0} y={0} w={SIZE} h={SIZE} fill={bg} />
          <div style={{ position: "absolute", inset: 0, background: `radial-gradient(40% 40% at 50% 40%, ${bg} 0%, rgba(0,0,0,0) 60%)`, opacity: 0.6 }} />
        </>
      );
    }
  ...
  const Eyes = () => {
    ...
    if (eyesIdx === 3) {
      ...
      return (<><Diamond cx={box.x + box.w*0.30} /><Diamond cx={box.x + box.w*0.70} /></>);
    }
    return (<><Circle cx={box.x + box.w*0.34} cy={box.y + box.h*0.44} r={22} fill={eye} /><Rect x={box.x + box.w*0.58} y={box.y + box.h*0.40} w={48} h={48} r={10} fill={eye} /></>);
  };
  ...
  const Arms = () => {
    if (armIdx === 0) {
      return (
        <>
          <div style={{ position: "absolute", left: box.x - 40, top: box.y + box.h*0.50, width: 120, height: 12, background: stroke, opacity: 0.9, borderRadius: 8, filter: "blur(0.2px)" }} />
          <div style={{ position: "absolute", left: box.x + box.w - 80, top: box.y + box.h*0.50, width: 120, height: 12, background: stroke, opacity: 0.9, borderRadius: 8, filter: "blur(0.2px)" }} />
  ...
  const Legs = () => {
    if (legIdx === 0) return (<><Rect x={box.x + 40} y={box.y + box.h - 24} w={120} h={24} r={12} fill={chassis} /><Rect x={box.x + box.w - 160} y={box.y + box.h - 24} w={120} h={24} r={12} fill={chassis} /></>);
    if (legIdx === 1) return (<><Rect x={box.x + box.w*0.44} y={box.y + box.h - 54} w={16} h={54} fill={chassis} /><Rect x={box.x + box.w*0.56} y={box.y + box.h - 54} w={16} h={54} fill={chassis} /></>);
    return (<><Circle cx={box.x + 120} cy={box.y + box.h + 6} r={22} fill={chassis} /><Circle cx={box.x + box.w - 120} cy={box.y + box.h + 6} r={22} fill={chassis} /></>);
  };
  ...
  const Antenna = () =>
    antenna ? (
      <>
        <Rect x={box.x + box.w/2 - 4} y={box.y - 42} w={8} h={42} r={4} fill={stroke} shadow={`0 0 0 ${stroke}`} />
  ...
  const Cube = () => (
    <>
      <Rect x={box.x + box.w - 70} y={box.y} w={70} h={box.h} r={rx} fill={"rgba(0,0,0,0.20)"} opacity={0.8} />
      <Rect x={box.x} y={box.y} w={box.w} h={box.h} r={rx} fill={body} strokeColor={stroke} strokeWidth={12} shadow={`inset 0 0 0 3px ${stroke}44, 0 20px 50px ${stroke}26`} />
      <Rect x={box.x} y={box.y} w={box.w} h={Math.max(56, Math.round(box.h * 0.18))} r={20} fill={chassis} opacity={0.32} />
    </>
  );

  // Footer ID label
  const Footer = () => (
    <div style={{ position: "absolute", left: PADDING, bottom: PADDING, fontSize: 28, color: accent, opacity: 0.75, fontFamily: "monospace" }}>
